...
async function init() {
  ...
  if (ok) {
    ...
    // Security/cleanup: verify-key stored in localStorage ko dashboard load ke 5s baad remove kar dein.
    // NOTE: admin flow uses this key only for verification; after load we rely on admin_verified flag + revalidation.
    window.setTimeout(() => {
      try {

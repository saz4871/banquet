    ...
    <div style="margin-top: 15px;">
      ...
      <div style="margin-top: 15px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px; margin-top: 5px;">
          <label><input id="banquet_avail_morning" type="checkbox"> Morning</label>
          <label><input id="banquet_avail_evening" type="checkbox"> Evening</label>
          <label><input id="banquet_avail_night" type="checkbox"> Night</label>
        </div>
      </div>
  ...
  <div style="margin-top: 15px;">
    ...
    <div style="margin-top: 15px;">
      ...
      <div style="margin-top: 15px;">
        <label>Avaibility</label>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px; margin-top: 5px;">
          <label><input id="hall_avail_morning" type="checkbox"> Morning</label>
          <label><input id="hall_avail_evening" type="checkbox"> Evening</label>
          <label><input id="hall_avail_night" type="checkbox"> Night</label>
        </div>
      </div>
  ...
  "master-record": `
    <div class="cardish">
      <div style="display: flex; justify-content: space-between; margin-bottom: 15px; align-items: center; gap: 12px; flex-wrap: wrap;">
        ...
        <div style="display:flex; gap:12px; align-items:center; flex-wrap: wrap;">
          <div class="calendar-month" data-month-label="master-record-date">
            <button type="button" class="calendar-month__chev" data-month-nav="prev" aria-label="Previous month">‹</button>
            <span class="calendar-month__label" id="masterRecordMonthLabel" data-calendar-month-label="master-record-date">July 2026</span>
          ...
          <select id="masterVendorSpreadsheetStatusFilter" style="padding: 8px; border-radius: 8px; border: 1px solid var(--border); background: rgba(0,0,0,0.2); color: var(--text);">
          ...
          <input type="text" id="masterVendorSpreadsheetSearch" placeholder="Search records..." style="padding: 8px; border-radius: 8px; border: 1px solid var(--border); background: rgba(0,0,0,0.2); color: var(--text);">
        </div>

        <div style="display:flex; gap:10px; align-items:center;">
          <button id="masterVendorSpreadsheetRefreshBtn" type="button" class="a-btn">Refresh</button>
          <button type="button" onclick="exportTableToPDF('masterRecordTable')" class="a-btn">Export PDF</button>
        </div>
      </div>

      <div style="overflow-x: auto; padding-bottom: 10px;">
        <table id="masterRecordTable" style="width: 100%; border-collapse: collapse; margin-top: 15px; text-align: left; color: var(--text); white-space: nowrap;">
          <thead>
            <tr style="border-bottom: 1px solid var(--border);">
              <th style="padding: 10px;">User ID</th>
              <th style="padding: 10px;">User Name</th>
              <th style="padding: 10px;">User Contact</th>
              <th style="padding: 10px;">Venue ID</th>
          ...
          <tbody id="masterVendorRequestsTbody"></tbody>
        </table>
      </div>
    </div>
`,
...
function renderView(key) { 
  // helper for master-record view
  // (declare once as function scope)
...
// Naya Update: Banquet management view ke liye engine activate karein
  if (key === "master-record") {
    // Render Master Record with Vendor Spreadsheet-style table.
    // Import/export mismatch fix: dynamically import the module that defines the function.
    import("./blackboxai_master_record_vendor_spreadsheet.js")
      .then((m) => {
        if (typeof m.initMasterRecordVendorSpreadsheet === "function") {
          return m.initMasterRecordVendorSpreadsheet();
        }
        throw new Error("initMasterRecordVendorSpreadsheet not exported from module");
      })
...
function validateBanquetManagementForm() {
  ...
  // Avaibility: at least one checkbox checked
  const availIds = [
    'banquet_avail_morning',
    'banquet_avail_evening',
    'banquet_avail_night',
  ];

  const anyAvailChecked = availIds.some((id) => {
    const cb = getEl(id);
    return cb ? !!cb.checked : false;
  });

  if (!anyAvailChecked) {
    const firstCb = getEl(availIds[0]);
    markBad(firstCb);
    return {
      ok: false,
      firstEl: firstCb,
      message: 'Select at least one Avaibility checkbox (Morning/Evening/Night).',
    };
  }
...
function getBanquetPayloadFromForm() {
  ...
  const availMorning = getChecked('banquet_avail_morning');
  const availEvening = getChecked('banquet_avail_evening');
  const availNight = getChecked('banquet_avail_night');
...
async function loadBanquetForEditingByPath(dataPath) {
  ...
  const r = await decryptDeep(snap.val() || {});
  ...
  // Avaibility (Morning/Evening/Night) -> checkboxes
  const availObj = r.avaibility ?? r.availability ?? {};
  const setAvailCb = (id, v) => {
    const cb = document.getElementById(id);
    if (cb) cb.checked = checkboxCheckedFromValue(v);
  };
  setAvailCb('banquet_avail_morning', availObj?.Morning ?? availObj?.morning ?? availObj?.MORNING);
  setAvailCb('banquet_avail_evening', availObj?.Evening ?? availObj?.evening ?? availObj?.EVENING);
  setAvailCb('banquet_avail_night', availObj?.Night ?? availObj?.night ?? availObj?.NIGHT);
...
async function loadHallForEditingByPath(dataPath) {
  ...
  // Avaibility -> checkboxes
  const availObj = r.avaibility ?? r.availability ?? {};
  const setAvailCb = (id, v) => {
    const cb = document.getElementById(id);
    if (cb) cb.checked = checkboxCheckedFromValue(v);
  };
  setAvailCb('hall_avail_morning', availObj?.Morning ?? availObj?.morning ?? availObj?.MORNING);
  setAvailCb('hall_avail_evening', availObj?.Evening ?? availObj?.evening ?? availObj?.EVENING);
  setAvailCb('hall_avail_night', availObj?.Night ?? availObj?.night ?? availObj?.NIGHT);
...
function getHallPayloadFromForm() {
  ...
  const availMorning = !!document.getElementById('hall_avail_morning')?.checked;
  const availEvening = !!document.getElementById('hall_avail_evening')?.checked;
  const availNight = !!document.getElementById('hall_avail_night')?.checked;
  ...
  return {
    ...
    // Avaibility (Morning/Evening/Night)
    avaibility: {
      Morning: availMorning,
      Evening: availEvening,
      Night: availNight,
  ...
  navItems.forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (key === "banquet-spreadsheet") {
        try { await loadBanquetSpreadsheetRows(); } catch (e) { console.error(e); }
      }
      if (key === "hall-spreadsheet") {
        try { await loadHallSpreadsheetRows(); } catch (e) { console.error(e); }
      }
...
window.exportTableToPDF = async function (tableId) {
  ...
  // SIRF 'Actions' header wale column ko target karein
  const headerCells = clonedTable.querySelectorAll('th');
  let actionsIndex = -1;

  // Actions column ka index find karein
  ...
  // Agar 'Actions' column mil jaye, toh sirf usay remove karein
  if (actionsIndex !== -1) {
    const rows = clonedTable.querySelectorAll('tr');
    rows.forEach((tr) => {
      if (tr.children.length > actionsIndex) {
        tr.deleteCell(actionsIndex);
      }
    });
  }
    ...
    <div class="cardish">
      ...
      <div style="overflow-x: auto; padding-bottom: 10px;">
        <table id="masterRecordTable" style="width: 100%; border-collapse: collapse; margin-top: 15px; text-align: left; color: var(--text); white-space: nowrap;">
          <thead>
            <tr style="border-bottom: 1px solid var(--border);">
              <th style="padding: 10px;">Client UID</th>
              <th style="padding: 10px;">Name</th>
              <th style="padding: 10px;">Contact</th>
              <th style="padding: 10px;">Venue UID</th>
...
>>>>>>> 2ae7c7e30a671b21403f0567f89bcb8091ede2b2
});
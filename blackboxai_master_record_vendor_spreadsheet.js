...
export async function initMasterRecordVendorSpreadsheet() {
  ...
  const renderEmpty = (msg) => {
    tbody.innerHTML = `
      <tr>
        <td colspan="8" style="padding:14px;color:rgba(229,231,235,0.7);font-weight:800;">${msg}</td>
      </tr>
    `;
  };
  ...
  const applyFilters = () => {
    ...
    const filtered = rows.filter((row) => {
      ...
      if (q) {
        // Search across every displayed field. This allows searching by user ID,
        // user name/contact, venue ID/name, target/requested date, or action.
        const hay = [
          row.uid,
          row.name,
          row.contact,
      ...
      // Month navigation remains available for normal browsing.
      // When the user is actively searching, search must work across ALL months
      // so a target/request date from another month can still be found.
      if (!q && selectedYM) {
    ...
    tbody.innerHTML = filtered.map((row) => {
      ...
      const actionText = row.status || 'Pending';
      ...
      return `
        <tr style="border-bottom:1px solid rgba(251,113,133,0.18);">
          ...
          <td style="padding:10px;">${row.venueId}</td>
          <td style="padding:10px;">${row.venueName}</td>
          <td style="padding:10px;">${row.targetDate}</td>
          <td style="padding:10px;">${row.requestedDate}</td>
          <td style="padding:10px;">
            <span style="color:${sCol};font-weight:900;">${actionText}</span>
          </td>
        </tr>
      `;
    }).join('');
  ...
  const loadRows = async () => {
    ...
    for (const [id, r] of Object.entries(banquetData || {})) {
      if (!r) continue;
      const venueId = String(r.UID ?? r.uid ?? r.bank_UID ?? id).trim();
      if (!venueId) continue;
      venueById.set(venueId, {
        name: String(r.bankname ?? r.banquetName ?? r.name ?? ''),
        id: venueId,
      });
    }

    for (const [id, r] of Object.entries(hallData || {})) {
      if (!r) continue;
      const venueId = String(r.hall_UID ?? r.hallUID ?? r.UID ?? r.uid ?? id).trim();
      if (!venueId) continue;
      venueById.set(venueId, {
        name: String(r.hallname ?? r.bankname ?? r.name ?? ''),
        id: venueId,
      });
    }
    ...
    for (const [id, r] of Object.entries(userData || {})) {
      if (!r) continue;

      const venueUid = String(
        r.venueId ?? r.venueUID ?? r.venueuid ?? r.selectedAssetUid ?? r.assetUid ?? ''
      ).trim();

      const venue = venueById.get(venueUid);

      out.push({
        rawKey: id,
        uid: String(r.user_UID ?? r.useruid ?? r.UID ?? r.uid ?? id),
        name: String(r.clientname ?? r.name ?? r.username ?? r.userName ?? ''),
        contact: String(r.clientcontact ?? r.contact ?? r.phone ?? r.number ?? r.whatsapp ?? ''),
        targetDate: String(r.targetdate ?? r.targetDate ?? ''),
        requestedDate: String(r.requesteddate ?? r.requestedDate ?? r.requestDate ?? ''),
        venueName: String(venue?.name ?? r.venueName ?? r.banquetName ?? r.hallname ?? r.bankname ?? ''),
        venueId: String(venue?.id ?? venueUid ?? r.venueId ?? ''),
        status: String(r.status ?? r.approval_status ?? r.action ?? 'pending'),
        raw: r,
      });
    }
    ...
    rows = out;
    if (!rows.length) renderEmpty('No records to show.');

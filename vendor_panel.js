  ...
  if (viewKey === "vendor-management") {
    ...
    (async () => {
      try {
        ...
        // onValue will keep updating while vendor-management view is active.
        ...
        const pendingOnly = (statusStr) => {
          const x = normalizeStatus(statusStr);
          return x === "pending" || x.includes("pending") || x.includes("awaiting");
        };
        const approvedOnly = (statusStr) => {
          const x = normalizeStatus(statusStr);
          return x === "approved" || x.includes("approved");
        };

        // Pending card click handler (vendor-management -> vendor-spreadsheet with pending filter)
        const handlePendingCardClick = () => {
          try {
            localStorage.setItem('vendorSpreadsheetStatusPreset', 'pending');
          } catch (e) {}
          renderView('vendor-spreadsheet');

          // Wait for spreadsheet DOM to mount, then apply pending filter.
          setTimeout(() => {
            const filterSelect = document.getElementById('vendorSpreadsheetStatusFilter');
            if (filterSelect) {
    ...
    try {
      repaintQuickCardsIfPresent();
        ...
        const collectTitle = (item) =>
          item.title || item.bankname || item.name || item.uid || item.UID || '';
        ...
        if (tbodyEl) {
          tbodyEl.addEventListener("click", async (evt) => {
            ...
            try {
              ...
              // Ensure Vendor Management cards (Pending/Approved) reflect latest spreadsheet status
              // by re-rendering that view after DB update.
              try {
